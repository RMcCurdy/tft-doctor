...
import { OFFENSIVE_ITEM_IDS, SECONDARY_CARRY_THRESHOLD, SECONDARY_CARRY_MIN_ITEMS, isSummonedUnit } from "@/lib/constants";
...
export function identifyCarry(units: RiotUnit[]): CarryResult {
  ...
  const scored = units
    .filter((unit) => !isSummonedUnit(unit.character_id))
